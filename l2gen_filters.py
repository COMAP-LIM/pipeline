...
class Normalize_Gain(Filter):
    ...
    def lowpass_filter_safe(
        self, signal, fknee=0.01, alpha=4.0, samprate=50, num_threads=2
    ):
    ...
    def lowpass_filter(
        self, signal, fknee=0.01, alpha=4.0, samprate=50, num_threads=-1
    ):
        Ntod = signal.shape[-1]
    ...
    def lowpass_filter_new(
        self, signal, fastlen, fknee=0.01, alpha=4.0, samprate=50, num_threads=-1
    ):
        Ntod = signal.shape[-1]
        signal_padded = np.zeros((1024, fastlen))
        ...
        if not self.use_ctypes:
            ...
            for feed in range(l2.Nfeeds):
                for sb in range(l2.Nsb):
                    ...
                    tod_lowpass = self.lowpass_filter(
                        l2.tod[feed, sb],
            ...
            for feed in range(l2.Nfeeds):
                for sb in range(l2.Nsb):
                    ...
                    l2_padded[:, l2.Ntod * 2 :] = np.nanmean(
                        l2.tod[feed, sb, :, :400], axis=-1
                    )[:, None]
...
class Decimation(Filter):
    ...
    def run(self, l2):
        ...
        tod_decimated = np.zeros(
            (l2.tod.shape[0], l2.tod.shape[1], self.Nfreqs, l2.tod.shape[3]),
            dtype=np.float32,
        )
        for freq in range(self.Nfreqs):
            tod_decimated[:, :, freq, :] = np.nansum(
                l2.tod[:, :, freq * self.dnu : (freq + 1) * self.dnu, :]
                * weight[:, :, freq * self.dnu : (freq + 1) * self.dnu, None],
                axis=2,
            )
            tod_decimated[:, :, freq, :] /= np.nansum(
                weight[:, :, freq * self.dnu : (freq + 1) * self.dnu], axis=2
            )[:, :, None]
        l2.tod = tod_decimated
        l2.freqmask_decimated = np.zeros((l2.Nfeeds, l2.Nsb, self.Nfreqs), dtype=bool)
        for freq in range(self.Nfreqs):
            l2.freqmask_decimated[:, :, freq] = l2.freqmask[
                :, :, freq * self.dnu : (freq + 1) * self.dnu
            ].any(axis=-1)
        ...
        for ifreq in range(self.Nfreqs):
            delta_nu = np.nansum(
                l2.freqmask[:, :, self.dnu * ifreq : self.dnu * (ifreq + 1)], axis=-1
            )
            tsys_decimated[:, :, ifreq] = np.sqrt(
                delta_nu
                / np.nansum(
                    l2.freqmask[:, :, self.dnu * ifreq : self.dnu * (ifreq + 1)]
                    / l2.Tsys[:, :, self.dnu * ifreq : self.dnu * (ifreq + 1)] ** 2,
                    axis=-1,
                )
            )
...
class Pointing_Template_Subtraction(Filter):
    ...
    def run(self, l2):
        ...
        if not self.use_ctypes:
                    ...
                    for freq in range(l2.Nfreqs):
                        if l2.scantype == "ces":
                            if np.isfinite(l2.tod[feed, sb, freq]).all():
                                (d, c), _ = curve_fit(
                                    az_func, l2.az[feed], l2.tod[feed, sb, freq], (d, c)
                                )
                            else:
                                d, c = 0, 0
                        else:
                            if np.isfinite(l2.tod[feed, sb, freq]).all():
                                (g, d, c), _ = curve_fit(
                                    az_el_func,
                                    (
                                        1.0 / np.sin(l2.el[feed] * np.pi / 180.0),
                                        l2.az[feed],
                                    ),
                                    l2.tod[feed, sb, freq],
                                    (g, d, c),
                                ...
                                g, d, c = 0, 0, 0
                        l2.tod[feed, sb, freq] = l2.tod[
                            feed, sb, freq
                        ] - az_el_template(feed, g, d, c)
            ...
            float64_array1 = np.ctypeslib.ndpointer(
                dtype=ctypes.c_double, ndim=1, flags="contiguous"
            )
            pointinglib.az_fit.argtypes = [
                float64_array1,
                float32_array3,
                ctypes.c_int,
                ctypes.c_int,
            ...
            pointinglib.az_el_fit.argtypes = [
                float64_array1,
                float64_array1,
                float32_array3,
                ctypes.c_int,
            ...
            if l2.scantype == "ces":
                for ifeed in range(l2.Nfeeds):
                    ...
                    l2.tod[ifeed] -= (
                        l2.az[ifeed][None, None, :] * d_est[:, :, None]
                        + c_est[:, :, None]
                    )
                    l2.tofile_dict["el_az_amp"][ifeed, :, :, 0] = np.zeros(
                        (l2.Nsb, l2.Nfreqs)
                    )
...
class Polynomial_filter(Filter):
    ...
    def run(self, l2, masking_run=False):
        if masking_run and l2.params.write_C_matrix:
            l2.tofile_dict["C_template_polyfilter"] = np.zeros(
                (l2.Nfeeds, 2, 2 * l2.Nfreqs, 2 * l2.Nfreqs)
            )
        ...
        if not self.use_ctypes:
            ...
            for ifeed in range(l2.Nfeeds):
                for sb in range(4):
                    for idx in range(l2.Ntod):
                        ...
                        tod_local[
                            ~np.isfinite(tod_local)
                        ] = 0  # polyfit doesn't allow nans.
                        try:
                            c1[ifeed, sb, idx], c0[ifeed, sb, idx] = np.polyfit(
                                sb_freqs, tod_local, 1, w=l2.freqmask[ifeed, sb]
                            )
                            l2.tod[ifeed, sb, :, idx] = (
                                l2.tod[ifeed, sb, :, idx]
                                - c1[ifeed, sb, idx] * sb_freqs
                                - c0[ifeed, sb, idx]
                            )
                        except:
                            pass
            ...
            for ifeed in range(l2.Nfeeds):
                for sb in range(4):
                    ...
                    l2.tod[ifeed, sb, :, :] = (
                        l2.tod[ifeed, sb, :, :]
                        - c1[ifeed, sb, None, :] * sb_freqs[:, None]
                        - c0[ifeed, sb, None, :]
                    )  # Future improvement: Move into C.
            ...
            for ifeed in range(l2.Nfeeds):
                for isb in range(l2.Nsb):
                    ...
                    if (b != 0).any():
                        ...
                        l2.corr_template[
                            ifeed,
                            l2.Nfreqs * isb : l2.Nfreqs * (isb + 1),
                            l2.Nfreqs * isb : l2.Nfreqs * (isb + 1),
                        ] += template
                        if self.params.write_C_matrix:
                            l2.tofile_dict["C_template_polyfilter"][
                                ifeed,
                                isb // 2,
                                isb % 2 * l2.Nfreqs : (isb % 2 + 1) * l2.Nfreqs,
                                isb % 2 * l2.Nfreqs : (isb % 2 + 1) * l2.Nfreqs,
                            ] = template
...
class Frequency_filter(Filter):
    ...
    def binned_PS(self, data, Nbins=100):
        ...
        freq_bins = np.linspace(
            np.log10(freqs[0]) - 1e-6, np.log10(freqs[-1]) + 1e-6, Nbins
        )
    ...
    def gain_temp_sep(self, y, P, F, sigma0_g=None, fknee_g=None, alpha_g=None):
        Nfreqs, Ntod = y.shape
        if (
            sigma0_g and fknee_g and alpha_g
        ):  # Set any of the prior parameters to None or False to skip prior.
    ...
    def run(self, l2, masking_run=False):
        ...
        if (
            self.params.freqfilter_full_feed
        ):  # Testing of all-sb instead of per-sb for gain.
            l2.tofile_dict["freqfilter_P"] = np.zeros(
                (l2.Nfeeds, l2.Nsb * l2.Nfreqs, 2)
            )
            l2.tofile_dict["freqfilter_F"] = np.zeros(
                (l2.Nfeeds, l2.Nsb * l2.Nfreqs, 1)
            )
            ...
            for feed in range(l2.Nfeeds):
                ...
                for sb in range(l2.Nsb):
                    l2.tod[feed, sb] = (
                        l2.tod[feed, sb]
                        - F[l2.Nfreqs * sb : l2.Nfreqs * (sb + 1)].dot(a)
                        - P[l2.Nfreqs * sb : l2.Nfreqs * (sb + 1)].dot(m)
                    )
                ...
                if masking_run:
                    ...
                    def var_est(fourier_func):
                        ...
                        if l2.Ntod % 2 == 0:
                            var_est = (
                                np.sum(fourier_func[1:-1]) + fourier_func[-1] / 2
                            ) / div_factor
                        else:
                    ...
                    F1 = F_reduced - F
                    F0 = F_reduced
                    Z = np.eye(l2.Nfreqs * 4, l2.Nfreqs * 4) - P.dot(
                        np.linalg.pinv(P_reduced.T.dot(P_reduced))
                    ).dot(P_reduced.T)
                    Z0 = np.eye(l2.Nfreqs * 4, l2.Nfreqs * 4) - P_reduced.dot(
                        np.linalg.pinv(P_reduced.T.dot(P_reduced))
                    ).dot(P_reduced.T)
                    ...
                    if self.use_prior:
                        sigma_1 = np.nanmean(
                            np.std(
                                l2.tod[feed, sb, :, 1:] - l2.tod[feed, sb, :, :-1],
                                axis=-1,
                            )
                        ) / np.sqrt(2)
            ...
            l2.tofile_dict["freqfilter_P_reduced"] = np.zeros(
                (l2.Nfeeds, l2.Nsb, l2.Nfreqs, 2)
            )
            l2.tofile_dict["freqfilter_F_reduced"] = np.zeros(
                (l2.Nfeeds, l2.Nsb, l2.Nfreqs, 1)
            )
            ...
            if masking_run and self.params.write_C_matrix:
                l2.tofile_dict["C_template_freqfilter"] = np.zeros(
                    (l2.Nfeeds, 2, 2 * l2.Nfreqs, 2 * l2.Nfreqs)
                )
                ...
                for sb in range(l2.Nsb):
                    ...
                    if masking_run:
                        ...
                        if (a != 0).any() or (m != 0).any():
                            ...
                            F1 = F_reduced - F
                            F0 = F_reduced
                            Z = np.eye(l2.Nfreqs, l2.Nfreqs) - P.dot(
                                np.linalg.pinv(P_reduced.T.dot(P_reduced))
                            ).dot(P_reduced.T)
                            Z0 = np.eye(l2.Nfreqs, l2.Nfreqs) - P_reduced.dot(
                                np.linalg.pinv(P_reduced.T.dot(P_reduced))
                            ).dot(P_reduced.T)
                            z0 = F_reduced.T.dot(Z0).dot(F_reduced)[0]
                            ...
                            l2.corr_template[
                                feed,
                                sb * l2.Nfreqs : (sb + 1) * l2.Nfreqs,
                                sb * l2.Nfreqs : (sb + 1) * l2.Nfreqs,
                            ] += template
                            if self.params.write_C_matrix:
                                l2.tofile_dict["C_template_freqfilter"][
                                    feed,
                                    sb // 2,
                                    sb % 2 * l2.Nfreqs : (sb % 2 + 1) * l2.Nfreqs,
                                    sb % 2 * l2.Nfreqs : (sb % 2 + 1) * l2.Nfreqs,
                                ] += template
                    ...
                    l2.tofile_dict["freqfilter_a_m_corr"][feed, sb] = np.sum(
                        (a - np.mean(a, axis=-1)[:, None])
                        * (m - np.mean(m, axis=-1)[:, None]),
                        axis=-1,
                    ) / (m.shape[-1] * np.std(a, axis=-1) * np.std(m, axis=-1))
    ...
    def run(self, l2, masking_run=False):
        if not self.use_ctypes:
            ...
            for ifeed in range(l2.Nfeeds):
                ...
                l2.tod[ifeed] = l2.tod[ifeed] - np.sum(
                    ak[:, :, None, :] * comps[None, None, :, :], axis=-1
                )
                pca_ampl[:, ifeed] = np.transpose(ak, (2, 0, 1))
            l2.tofile_dict[
                "pca_ampl"
            ] = pca_ampl  # [::-1]  # Scipy gives smallest eigenvalues first, we want largest first.
            ...
            p = l2.Ntod
            C_LIB_PATH = (
                "/mn/stornext/d22/cmbco/comap/jonas/l2gen_python/C_libs/PCA/PCAlib.so.1"
            )
            ...
            bool_array1 = np.ctypeslib.ndpointer(
                dtype=ctypes.c_bool, ndim=1, flags="contiguous"
            )
            PCAlib.PCA.argtypes = [
                float32_array2,
                bool_array1,
            ...
            PCAlib.subtract_eigcomp.argtypes = [
                float32_array4,
                float64_array1,
                float64_array3,
                ctypes.c_long,
            ...
            for i in range(self.max_pca_comp):
                ...
                if M.shape[0] > 10:
                    ...
                    PCAlib.subtract_eigcomp(
                        l2.tod, r, ak, l2.Nfeeds * l2.Nsb * l2.Nfreqs, l2.Ntod
                    )
                    pca_ampl[i] = ak
...
class PCA_feed_filter(Filter):
    ...
    def run(self, l2, masking_run=False):
        ...
        if masking_run and self.params.write_C_matrix:
            l2.tofile_dict["C_template_pcaf"] = np.zeros(
                (l2.Nfeeds, 2, 2 * l2.Nfreqs, 2 * l2.Nfreqs)
            )
        ...
        if not self.use_ctypes:
            ...
            max_iter = self.params.pca_max_iter
            err_tol = self.params.pca_error_tol
            l2.tofile_dict["PCAf_err"] = np.zeros(
                (self.max_pca_comp, l2.Nfeeds, max_iter)
            )
            ...
            for ifeed in range(l2.Nfeeds):
                for icomp in range(self.max_pca_comp):
                    ...
                    lambda_treshold = (
                        1.2 * (np.sqrt(n_actual) + np.sqrt(p)) ** 2 * sigma0**2
                    )
                    ...
                    PCAlib.subtract_eigcomp(
                        l2.tod[ifeed], r, ak, l2.Nsb * l2.Nfreqs, l2.Ntod
                    )
        ...
        if masking_run:
            for ifeed in range(l2.Nfeeds):
                b = (
                    pca_ampl[:, ifeed]
                    .reshape((self.max_pca_comp, l2.Nsb * l2.Nfreqs))
                    .T
                )
                b[~np.isfinite(b)] = 0
                ...
                if l2.params.write_C_matrix:
                    l2.tofile_dict["C_template_pcaf"][ifeed, 0] += template[
                        l2.Nfreqs * 0 : l2.Nfreqs * 2, l2.Nfreqs * 0 : l2.Nfreqs * 2
                    ]
                    l2.tofile_dict["C_template_pcaf"][ifeed, 1] += template[
                        l2.Nfreqs * 2 : l2.Nfreqs * 4, l2.Nfreqs * 2 : l2.Nfreqs * 4
                    ]
...
class Masking(Filter):
    ...
    def run(self, l2):
        ...
        logging.debug(
            f"[{rank}] [{self.name}] Starting correlation calculations and masking..."
        )

        if (
            len(self.params.load_freqmask_path) > 0
        ):  # If not empty string, load freqmasks from this directory instead of computing it.
            path = os.path.join(self.params.load_freqmask_path, l2.fieldname)
            filename = os.path.join(path, l2.l2_filename + ".h5")
            if not os.path.isfile(filename):
                raise ValueError(
                    f"Specified l2 file for loading freqmask does not exist: {filename}."
                )
            ...
            l2.freqmask_reason[freqmask] += 2**l2.freqmask_counter
        ...
        Ntod = l2_local.Ntod
        premask_tofile_dict_keys = list(
            l2_local.tofile_dict.keys()
        )  # Store what keys where in the "tofile_dict" prior to premask filters.

        for (
            masking_filter
        ) in (
            l2.filter_list
        ):  # Look through filter list and see if any filter needs to be run prior to masking.
            if masking_filter.run_when_masking:
                ...
                logging.debug(
                    f"[{rank}] [{self.name}] Running local {masking_filter.name_long} for masking purposes..."
                )
                t0 = time.time()
                pt0 = time.process_time()
                ...
                if self.params.write_inter_files:
                    l2_local.write_level2_data(
                        name_extension=f"_premask_{masking_filter.name}"
                    )
                logging.debug(
                    f"[{rank}] [{self.name}] Finished local/masking {masking_filter.name_long} in {time.time()-t0:.1f} s. Process time: {time.process_time()-pt0:.1f} s."
                )

        for key in l2_local.tofile_dict.keys():
            if (
                not key in premask_tofile_dict_keys
            ):  # If something new was added to the tofile_dict of l2_local during the premask filters, we need to explicitly move it to the main l2 file not to lose them.
                l2.tofile_dict[f"premask_{key}"] = l2_local.tofile_dict[key]

        if self.params.write_C_matrix:  # For debuging purposes only.
            l2.tofile_dict["premask_C"] = np.zeros(
                (l2.Nfeeds, 2, l2.Nfreqs * 2, l2.Nfreqs * 2)
            )
            l2.tofile_dict["C"] = np.zeros((l2.Nfeeds, 2, l2.Nfreqs * 2, l2.Nfreqs * 2))
            l2.tofile_dict["C_template"] = np.zeros(
                (l2.Nfeeds, 2, l2.Nfreqs * 2, l2.Nfreqs * 2)
            )
        ...
        for ifeed in range(l2.tod.shape[0]):
            for ihalf in range(
                2
            ):  # Perform seperate analysis on each half of of the frequency band.
                tod = l2_local.tod[ifeed, ihalf * 2 : (ihalf + 1) * 2, :, :]
                tod = tod.reshape(
                    (2 * tod.shape[1], tod.shape[2])
                )  # Merge sb dim and freq dim.
                # Start from the already existing freqmasks.
                freqmask = l2.freqmask[ifeed, ihalf * 2 : (ihalf + 1) * 2].flatten()
                freqmask_reason = l2.freqmask_reason[
                    ifeed, ihalf * 2 : (ihalf + 1) * 2
                ].flatten()
                ...
                corr_template = l2_local.corr_template[
                    ifeed,
                    ihalf * l2.Nfreqs * 2 : (ihalf + 1) * l2.Nfreqs * 2,
                    ihalf * l2.Nfreqs * 2 : (ihalf + 1) * l2.Nfreqs * 2,
                ]
                ...
                for isb in range(2):
                    for ifreq in range(l2.Nfreqs):
                        max_corr[ifeed, ihalf * 2 + isb, ifreq] = np.nanmax(
                            np.abs(C[isb * l2.Nfreqs + ifreq]), axis=-1
                        )
                            ...
                            if i < j:
                                ...
                                if chi2 > Nsigma_chi2_box:
                                    ...
                                    for x in range(i * box_size, (i + 1) * box_size):
                                        if (
                                            freqmask_reason[x]
                                            & 2 ** (l2.freqmask_counter + ibox * 3 + 0)
                                            == 0
                                        ):
                                            freqmask_reason[x] += 2 ** (
                                                l2.freqmask_counter + ibox * 3 + 0
                                            )
                                ...
                                if prod / nprod > Nsigma_prod_box * np.sqrt(
                                    1.0 / nprod
                                ):
                                    freqmask[i * box_size : (i + 1) * box_size] = False
                                    for x in range(i * box_size, (i + 1) * box_size):
                                        if (
                                            freqmask_reason[x]
                                            & 2 ** (l2.freqmask_counter + ibox * 3 + 1)
                                            == 0
                                        ):
                                            freqmask_reason[x] += 2 ** (
                                                l2.freqmask_counter + ibox * 3 + 1
                                            )
                                    freqmask[j * box_size : (j + 1) * box_size] = False
                                    for x in range(j * box_size, (j + 1) * box_size):
                                        if (
                                ...
                                if np.abs(box_sum) / dof > Nsigma_mean_box * np.sqrt(
                                    1.0 / dof
                                ):
                                    freqmask[i * box_size : (i + 1) * box_size] = False
                                    for x in range(i * box_size, (i + 1) * box_size):
                                        if (
                                            freqmask_reason[x]
                                            & 2 ** (l2.freqmask_counter + ibox * 3 + 2)
                                            == 0
                                        ):
                                            freqmask_reason[x] += 2 ** (
                                                l2.freqmask_counter + ibox * 3 + 2
                                            )
                                    freqmask[j * box_size : (j + 1) * box_size] = False
                                    for x in range(j * box_size, (j + 1) * box_size):
                                        if (
                ...
                for istripe in range(len(self.stripe_sizes)):
                    ...
                    for i in range((2 * Nfreqs) // stripe_size):
                        ...
                        chi2 = (corr - dof) / np.sqrt(2 * dof)
                        chi2_matrix[
                            1, istripe, :, i * stripe_size : (i + 1) * stripe_size
                        ] = chi2
                ...
                l2.freqmask[ifeed, ihalf * 2 : (ihalf + 1) * 2] = freqmask.reshape(
                    (2, Nfreqs)
                )
                l2.freqmask_reason[
                    ifeed, ihalf * 2 : (ihalf + 1) * 2
                ] = freqmask_reason.reshape((2, Nfreqs))
        del l2_local
        ...
        if (
            int(l2.obsid) < 28136
        ):  # Newer obsids have different (overlapping) frequency grid which alleviates the aliasing problem.
            with h5py.File(
            ...
            l2.freqmask_reason[AB_mask[l2.feeds - 1] < 15] += 2**l2.freqmask_counter
            l2.freqmask_counter += 1
            l2.freqmask_reason_string.append("Aliasing suppression (AB_mask)")
            l2.freqmask_reason[leak_mask[l2.feeds - 1] < 15] += 2**l2.freqmask_counter
            l2.freqmask_counter += 1
        ...
        l2.freqmask_reason[var > (radiometer * std_max) ** 2] += (
            2**l2.freqmask_counter
        )
        l2.freqmask_counter += 1
        ...
        printstring = (
            f"[{rank}] [{l2.scanid}] [{self.name}] Acceptrate by feed and sideband:\n"
        )
        printstring += f"           all"
        for ifeed in range(l2.Nfeeds):
        ...
        print(printstring)
        print(
            f"[{rank}] [{self.name}] Finished correlation calculations and masking in {time.time()-t0:.1f} s. Process time: {time.process_time()-pt0:.1f} s."
        )
        ...
        if self.params.write_C_matrix:
            ...
            for i in range(l2.Nfeeds):
                ax[i, 0].imshow(
                    l2.tofile_dict["premask_C"][i, 0], cmap="bwr", vmin=-0.01, vmax=0.01
                )
                ax[i, 1].imshow(
                    l2.tofile_dict["premask_C"][i, 1], cmap="bwr", vmin=-0.01, vmax=0.01
                )
                ax[i, 2].imshow(
                    l2.tofile_dict["C"][i, 0], cmap="bwr", vmin=-0.01, vmax=0.01
                )
                ax[i, 3].imshow(
                    l2.tofile_dict["C"][i, 1], cmap="bwr", vmin=-0.01, vmax=0.01
                )
...
class Tsys_calc(Filter):
    ...
    def run(self, l2):
        ...
        for ifeed in range(l2.Nfeeds):
            feed = l2.feeds[ifeed]
            if (
                successful[feed - 1, 0] and successful[feed - 1, 1]
            ):  # Both calibrations successful.
                ...
                feed - 1, 0
            ]:  # Only first calibration successful: Use values from that one.
            ...
            l2.Tsys[ifeed, :, :] = (Thot_interp - Tcmb) / (
                Phot_interp / Pcold[ifeed] - 1
            )
...
class Calibration(Filter):
    ...
    def run(self, l2):
        ...
        l2.freqmask_reason[~np.isfinite(l2.Tsys)] = 2**l2.freqmask_counter
        l2.freqmask_counter += 1
        l2.freqmask_reason_string.append("Tsys NaN or inf")

        l2.freqmask[l2.Tsys < self.min_tsys] = False
        l2.freqmask_reason[l2.Tsys < self.min_tsys] = 2**l2.freqmask_counter
        l2.freqmask_counter += 1
        l2.freqmask_reason_string.append("Tsys < min_tsys")

        l2.freqmask[l2.Tsys > self.max_tsys] = False
        l2.freqmask_reason[l2.Tsys > self.max_tsys] = 2**l2.freqmask_counter
        l2.freqmask_counter += 1

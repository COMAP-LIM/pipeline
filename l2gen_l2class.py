...
class level2_file:
    ...
    def load_level1_data(self):
        with h5py.File(self.l1_filename, "r") as f:
            ...
            self.scan_start_idx = np.searchsorted(
                self.tod_times, self.mjd_start, side="left"
            )
            self.scan_stop_idx = np.searchsorted(
                self.tod_times, self.mjd_stop, side="right"
            )  # stop_idx is the first index NOT in the scan.
            self.scan_start_idx_hk = np.searchsorted(
                self.hk_times, self.mjd_start, side="left"
            )
            self.scan_stop_idx_hk = np.searchsorted(
                self.hk_times, self.mjd_stop, side="right"
            )
            ...
            self.samprate = 1.0 / (
                self.tod_times_seconds[1] - self.tod_times_seconds[0]
            )
            self.array_features = f["/hk/array/frame/features"][()]
            self.array_time = f["/hk/array/frame/utc"][()]
            self.az = f["/spectrometer/pixel_pointing/pixel_az"][
                :, self.scan_start_idx : self.scan_stop_idx
            ]
            self.el = f["/spectrometer/pixel_pointing/pixel_el"][
                :, self.scan_start_idx : self.scan_stop_idx
            ]
            self.ra = f["/spectrometer/pixel_pointing/pixel_ra"][
                :, self.scan_start_idx : self.scan_stop_idx
            ]
            self.dec = f["/spectrometer/pixel_pointing/pixel_dec"][
                :, self.scan_start_idx : self.scan_stop_idx
            ]
            self.tod = f["/spectrometer/tod"][
                :, :, :, self.scan_start_idx : self.scan_stop_idx
            ]
            ...
            self.tofile_dict = (
                {}
            )  # Dict for adding custom data, which will be written to level2 file.
            self.Nfeeds = self.tod.shape[0]
            self.Nsb = self.tod.shape[1]
            self.Nfreqs = self.tod.shape[2]
            self.Ntod = self.tod.shape[3]
            self.corr_template = np.zeros(
                (self.Nfeeds, self.Nsb * self.Nfreqs, self.Nsb * self.Nfreqs)
            )  # The correlated induced by different filters, to be subtracted in masking.
    ...
    def write_level2_data(self, name_extension=""):
        ...
        temp_outfilename = os.path.join(
            outpath, "." + self.l2_filename + name_extension + ".h5"
        )
        ...
        with h5py.File(temp_outfilename, "w") as f:
            ...
            f["freqmask_reason_string"] = np.array(
                self.freqmask_reason_string, dtype="S100"
            )
            f["cal_method"] = 2
            ...
            f["obsid"] = self.obsid
            f["sigma0"] = np.std(
                self.tod[:, :, :, 1:] - self.tod[:, :, :, :-1], axis=-1
            ) / np.sqrt(2)
            f["n_nan"] = self.n_nans
            f["chi2"] = (
                np.sum(self.tod**2, axis=-1) / f["sigma0"][()] ** 2 - self.Ntod
            ) / np.sqrt(2 * self.Ntod)
            ...
            with h5py.File(self.l1_filename, "r") as l1file:
                f["hk_airtemp"] = l1file["hk/array/weather/airTemperature"][
                    self.scan_start_idx_hk : self.scan_stop_idx_hk
                ]

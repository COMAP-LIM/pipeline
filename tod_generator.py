...
class TOD_Gen:
    ...
    def get_one_over_f_noise(
        self, wn, sigma, f=None, feed=0, alpha=-1.8, f_knee=0.6, channel=11, sideband=0
    ):
        """ """
        ...
        psd_corr = (np.abs(f) / f_knee) ** alpha
        psd_corr[0] = (
            1  # Setting the zero frequency to be the white noise mean when convolved
        )
        psd_corr = psd_corr * sigma0**2
    ...
    def get_pointing(self, feed=0):
        ...
        if (
            self.az is None
        ):  # In case of reading from a file where the parameters point_tel have been asked to be saved
        ...
        for f in range(len(data)):
            for i in range(len(data[0][:][:])):
                for j in range(0, len(data_down[0][0][:])):
                    ...
                    weighted_sum = np.sum(
                        data[f, i, start_idx:end_idx] * weights, axis=0
                    )
    ...
    def get_data_model(
        ...
        if (
            tground_files
        ):  # If ground pickup is to be found from reading a convolution file
            ...
            Tground = self.tground_interpolation(
                tground_files, nfreq=freqs, pointing=point
            )  # shape feed, freq, time
            ...
            if constant_ground is False:
                ...
                air_temp_interp, _, _ = self.interpolating_air_temp(
                    air_temp=hk_air_temp,
                    sys_time=hk_time,
                    hk_start=self.scan_start_idx_hk,
                    hk_end=self.scan_stop_idx_hk,
                )

                if BB_test:
                    # air_temp_interp = np.zeros_like(air_temp_interp) + 300
                    Blackbody_factor = (
                        air_temp_interp + 273.15
                    ) / 300  # self.tground_blackbody(air_temp_interp)/300 # Dividing by 300 as this is the assumed temp
                ...
                if BB_test == False:
                    Blackbody_factor = (
                        self.tground_blackbody(air_temp_interp) / 300
                    )  # Dividing by 300 as this is the assumed temp
                    # print(f'Blackbody_factor has shape {np.shape(Blackbody_factor)}')
                    Tground = Blackbody_factor * Tground
        ...
        for (
            ...
            components.items()
        ):  # Component is name of component, variable is the component data

            if variable.ndim < self.tod.ndim:
                var_dim = [
                    dim for i, dim in enumerate(variable.shape)
                ]  # Making a list of the dimensions of the variable. Ex. tod would be [feeds, sidebands, freqs, time]
                ...
                for i in range(
                    len(tod_dim)
                ):  # This loop compare the dimensions of the variable and makes sure to insert an empty axis in order to make all components compatible for
        ...
        if (
            corr_noise
        ):  # Adding correlated noise in the low-level simulation is optional
    ...
    def azimuth_template(
        self, az=None, az_0=None, K=0.5, feed=0, sideband=0, channel=11
    ):
        """
    ...
    def fits_file_ground(
        self,
        filename,
        pointing=None,
        scale=300,
        outfile=f"{tod_gen_folder}/figs/testing/correct_sigma/healpy_TOD_of_ground_pickup2_from_map",
        file_fits=True,
    ):
        """
        !!!! ISN'T IN THE MODEL ***
        ...
        if file_fits:
            m = hp.read_map(path)
            val = scale * hp.pixelfunc.get_interp_val(
                m, az, el, nest=False, lonlat=True
            )
            ...
            img = hp.projview(
                ...
                graticule_color="lightgray",
                graticule=True,
                rot_graticule=False,
                rot=[90, 0],
                custom_ytick_labels=[None, -90, -60, -30, 0, 30, 60, 90],
                custom_xtick_labels=[None, 0, 60, 120, 180, 240, 300, 360],
                unit="Brightness [K]",
                reuse_axes=True,
                cb_orientation="vertical",
            )

            self.newprojplot_with_sensible_units(
                ground_profile[0] + 180, ground_profile[1][::-1], color="lime", lw=2
            )
        ...
        plt.figtext(
            0.5,
            0.02,
            f"Temperature map of ground pickup. Horizon is blocked out in grey.",
            ha="center",
            fontsize=10,
            wrap=True,
        )
    ...
    def tground_interpolation(
        self, freq_files, nside=256, pointing=None, nfreq=4 * 1024
    ):
        """
        ...
        maps_at_pointing = (
            []
        )  # 3 maps in total: one at 26GHz, one at 30GHz and one at 34GHz
        ...
        for i in range(N_feed):
            for j in range(N_time):
                interp_maps[:, i, j] = np.interp(
                    frequencies, np.array([26, 30, 34]), maps_at_pointing[:, i, j]
                )

        # from frq, feed, time -> feed, freq, time
        interp_maps = np.array(
            (
                interp_maps[:divider, :],
                interp_maps[divider : 2 * divider, :],
                interp_maps[2 * divider : 3 * divider, :],
                interp_maps[3 * divider :, :],
            )
        )

        interp_maps = (
            interp_maps.transpose(2, 0, 1, 3) * 300
        )  # The convolution maps are in normalized units, where 300 is assumed to be the ground temp
    ...
    def interpolating_air_temp(self, air_temp, sys_time, hk_start, hk_end, tol=0.01):
        ...
        smooth_air = sp.ndimage.gaussian_filter1d(
            air_temp, sigma=200
        )  # Smoothing less 15 min
        interp_sys_time = np.arange(len(sys_time[hk_start:hk_end]))

        air_temp = air_temp[hk_start:hk_end]

        original_air_temp_interp = np.interp(
            time_array, interp_sys_time, air_temp
        )  # Unsmoothed interpolation, for comparison
        air_temp_interpolated = np.interp(
            time_array, interp_sys_time, smooth_air[hk_start:hk_end]
        )  # Smoothed and then interpolated, used in datamodel
    ...
    def tground_blackbody(self, air_temp_interpolated, test=False):
        ...
        T = bb(freqs[..., None]).to(
            u.K, equivalencies=u.brightness_temperature(freqs[..., None])
        )
